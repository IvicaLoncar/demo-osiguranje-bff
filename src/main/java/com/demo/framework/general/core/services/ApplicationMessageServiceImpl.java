package com.demo.framework.general.core.services;

import com.demo.framework.general.core.DAO.ApplicationMessageDAO;
import com.demo.framework.general.core.enums.CustomMessageLevel;
import com.demo.framework.general.core.enums.MessagesPoint;
import com.demo.framework.general.core.model.ApplicationMessage;
import com.demo.framework.general.core.model.DataValidation;
import com.demo.framework.general.core.services.DataValidationService;
import com.demo.framework.general.core.util.ApplicationMessageUtil;
import com.demo.framework.general.core.util.CompareUtil;
import com.demo.framework.general.core.util.CustomData;
import com.demo.framework.general.core.util.CustomDummy;
import com.demo.framework.general.core.util.CustomMessage;
import com.demo.framework.general.core.util.DataValidationUtil;
import com.demo.framework.general.core.util.HandleTransaction;
import com.demo.framework.general.core.util.MessagesHandler;
import com.demo.framework.general.core.util.UnpivotedData;
import com.github.fge.jsonpatch.JsonPatch;

import java.util.ArrayList;
import java.util.List;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.slf4j.MDC;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import org.springframework.transaction.PlatformTransactionManager;
import org.springframework.transaction.TransactionStatus;
import org.springframework.transaction.support.DefaultTransactionDefinition;


@Service
public class ApplicationMessageServiceImpl implements ApplicationMessageService
{
  private static final Logger LOGGER = LoggerFactory.getLogger(ApplicationMessageServiceImpl.class);


  private List<ApplicationMessage> applicationMessages;


  @Autowired
  private DataValidationService dataValidationService;

  @Autowired
  private PlatformTransactionManager transactionManager;

  @Autowired
  private ApplicationMessageExtService applicationMessageExtService;

  @Autowired
  private ApplicationMessageDAO applicationMessageDAO;



  public ApplicationMessageServiceImpl()
  {
    if (this.applicationMessages == null) this.applicationMessages = new ArrayList<ApplicationMessage>();

  }


  @Override
  public CustomData<CustomDummy> handleMessages(ApplicationMessage applicationMessage, MessagesPoint messagesPoint)
  {
    return this.handleMessages(applicationMessage.unpivot(), messagesPoint);
  }


  @Override
  public CustomData<CustomDummy> handleMessages(UnpivotedData unpivotedApplicationMessage, MessagesPoint messagesPoint)
  {
    CustomData<CustomDummy> messagesStatus = new CustomData<CustomDummy>();

    MessagesHandler messagesHandler = new MessagesHandler();

    CustomData<List<DataValidation>> dataValidations = this.dataValidationService.get(1, 1000, null, null);
    List<DataValidation> dataValidationBetweenPoints = null;

    StringBuilder preparedMessage = null;

    boolean checkPassed = true;

    if (messagesHandler.proceed() && messagesPoint == MessagesPoint.BeforeSelect)
    {
      dataValidationBetweenPoints = DataValidationUtil.extract(dataValidations.getData(), messagesPoint, 0, 10000);
      if (dataValidationBetweenPoints.size() > 0)
      {
        messagesHandler.addMessages(
          "com.example.demo.general.framework.services.ApplicationMessageServiceImpl.messageGroup0_10000",
          this.applicationMessageExtService.handleExtMessages(unpivotedApplicationMessage, dataValidationBetweenPoints, messagesPoint));
      }
    }

    if (messagesHandler.proceed() && messagesPoint == MessagesPoint.BeforeInsert)
    {
      dataValidationBetweenPoints = DataValidationUtil.extract(dataValidations.getData(), messagesPoint, 0, 10000);
      if (dataValidationBetweenPoints.size() > 0)
      {
        messagesHandler.addMessages(
          "com.example.demo.general.framework.services.ApplicationMessageServiceImpl.messageGroup0_10000",
          this.applicationMessageExtService.handleExtMessages(unpivotedApplicationMessage, dataValidationBetweenPoints, messagesPoint));
      }
    }

    if (messagesHandler.proceed() && messagesPoint == MessagesPoint.BeforeUpdate)
    {
      dataValidationBetweenPoints = DataValidationUtil.extract(dataValidations.getData(), messagesPoint, 0, 10000);
      if (dataValidationBetweenPoints.size() > 0)
      {
        messagesHandler.addMessages(
          "com.example.demo.general.framework.services.ApplicationMessageServiceImpl.messageGroup0_10000",
          this.applicationMessageExtService.handleExtMessages(unpivotedApplicationMessage, dataValidationBetweenPoints, messagesPoint));
      }
    }

    if (messagesHandler.proceed() && messagesPoint == MessagesPoint.BeforeDelete)
    {
      dataValidationBetweenPoints = DataValidationUtil.extract(dataValidations.getData(), messagesPoint, 0, 10000);
      if (dataValidationBetweenPoints.size() > 0)
      {
        messagesHandler.addMessages(
          "com.example.demo.general.framework.services.ApplicationMessageServiceImpl.messageGroup0_10000",
          this.applicationMessageExtService.handleExtMessages(unpivotedApplicationMessage, dataValidationBetweenPoints, messagesPoint));
      }
    }

    if (messagesHandler.proceed() && messagesPoint == MessagesPoint.AfterSelect)
    {
      dataValidationBetweenPoints = DataValidationUtil.extract(dataValidations.getData(), messagesPoint, 0, 10000);
      if (dataValidationBetweenPoints.size() > 0)
      {
        messagesHandler.addMessages(
          "com.example.demo.general.framework.services.ApplicationMessageServiceImpl.messageGroup0_10000",
          this.applicationMessageExtService.handleExtMessages(unpivotedApplicationMessage, dataValidationBetweenPoints, messagesPoint));
      }
    }

    if (messagesHandler.proceed() && messagesPoint == MessagesPoint.AfterInsert)
    {
      dataValidationBetweenPoints = DataValidationUtil.extract(dataValidations.getData(), messagesPoint, 0, 10000);
      if (dataValidationBetweenPoints.size() > 0)
      {
        messagesHandler.addMessages(
          "com.example.demo.general.framework.services.ApplicationMessageServiceImpl.messageGroup0_10000",
          this.applicationMessageExtService.handleExtMessages(unpivotedApplicationMessage, dataValidationBetweenPoints, messagesPoint));
      }
    }

    if (messagesHandler.proceed() && messagesPoint == MessagesPoint.AfterUpdate)
    {
      dataValidationBetweenPoints = DataValidationUtil.extract(dataValidations.getData(), messagesPoint, 0, 10000);
      if (dataValidationBetweenPoints.size() > 0)
      {
        messagesHandler.addMessages(
          "com.example.demo.general.framework.services.ApplicationMessageServiceImpl.messageGroup0_10000",
          this.applicationMessageExtService.handleExtMessages(unpivotedApplicationMessage, dataValidationBetweenPoints, messagesPoint));
      }
    }

    if (messagesHandler.proceed() && messagesPoint == MessagesPoint.AfterDelete)
    {
      dataValidationBetweenPoints = DataValidationUtil.extract(dataValidations.getData(), messagesPoint, 0, 10000);
      if (dataValidationBetweenPoints.size() > 0)
      {
        messagesHandler.addMessages(
          "com.example.demo.general.framework.services.ApplicationMessageServiceImpl.messageGroup0_10000",
          this.applicationMessageExtService.handleExtMessages(unpivotedApplicationMessage, dataValidationBetweenPoints, messagesPoint));
      }
    }

    messagesStatus.appendMessages(messagesHandler);

    return messagesStatus;
  }


  @Override
  public CustomData<ApplicationMessage> get(Long id)
  {
    MessagesHandler messagesHandler = new MessagesHandler();

    CustomData<List<ApplicationMessage>> fetchedRows = null;

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
       		"com.example.demo.general.framework.ApplicationMessageServiceImpl.get.messagesSelect",
          (fetchedRows = this.applicationMessageDAO.select(1, 1, "applicationMessageID_" + Long.toString(id), null)));
    }

    CustomData<ApplicationMessage> returnRow = new CustomData<ApplicationMessage>();

    if (fetchedRows != null)
    {
    	if (fetchedRows.getData() != null && fetchedRows.getData().size() > 0)
    	{
    		returnRow.setData(fetchedRows.getData().get(0));
    	}
    	else
    	{
    		messagesHandler.setResourceNotFound(true);
    	}
    }

    returnRow.appendMessages(messagesHandler);

    return returnRow;
  }


  @Override
  public CustomData<List<ApplicationMessage>> get(Integer pageNumber, Integer rowPerPage, String filter, String sort)
  {
    MessagesHandler messagesHandler = new MessagesHandler();

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
       		"com.example.demo.general.framework.ApplicationMessageServiceImpl.get.messagesBeforeSelect",
      		this.handleMessages(new UnpivotedData(), MessagesPoint.BeforeSelect));
    }

    CustomData<List<ApplicationMessage>> fetchedRows = null;

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
       		"com.example.demo.general.framework.ApplicationMessageServiceImpl.get.messagesSelect",
          (fetchedRows = this.applicationMessageDAO.select(pageNumber, rowPerPage, filter, sort)));
    }

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
       		"com.example.demo.general.framework.ApplicationMessageServiceImpl.get.messagesAfterSelect",
      		this.handleMessages(new UnpivotedData(), MessagesPoint.AfterSelect));
    }

    CustomData<List<ApplicationMessage>> returnRows = new CustomData<List<ApplicationMessage>>();

    if (fetchedRows != null) returnRows.setData(fetchedRows.getData());

    returnRows.appendMessages(messagesHandler);

    return returnRows;
  }


  @Override
  public CustomData<ApplicationMessage> post(ApplicationMessage applicationMessage)
  {
    return this.post(applicationMessage, HandleTransaction.NO);
  }


  @Override
  public CustomData<ApplicationMessage> post(ApplicationMessage applicationMessage, HandleTransaction handleTransaction)
  {
    TransactionStatus transactionStatus = null;

    MessagesHandler messagesHandler = new MessagesHandler();

    if (handleTransaction == HandleTransaction.YES)
    {
      DefaultTransactionDefinition dtt = new DefaultTransactionDefinition();
      transactionStatus = this.transactionManager.getTransaction(dtt);
    }

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
       		"com.example.demo.general.framework.services.ApplicationMessageServiceImpl.post.messagesBeforeInsert",
      		this.handleMessages(applicationMessage, MessagesPoint.BeforeInsert));
    }

    CustomData<Long> lastId = null;

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
       		"com.example.demo.general.framework.services.ApplicationMessageServiceImpl.post.messagesGetLastId",
          (lastId = this.applicationMessageDAO.getLastId()));
      applicationMessage.setApplicationMessageId(lastId.getData() + 1);
    }

    CustomData<ApplicationMessage> insertedRowStatus = null;

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
       		"com.example.demo.general.framework.services.ApplicationMessageServiceImpl.post.messagesInsert",
          (insertedRowStatus = this.applicationMessageDAO.insert(applicationMessage)));
    }

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
       		"com.example.demo.general.framework.services.ApplicationMessageServiceImpl.post.messagesAfterInsert",
      		this.handleMessages(applicationMessage, MessagesPoint.AfterInsert));
    }

    if (handleTransaction == HandleTransaction.YES)
    {
      if (messagesHandler.markedForRollback())
      {
        System.out.println("rollback trans");
        transactionManager.rollback(transactionStatus);
      }
      else
      {
        System.out.println("commit trans");
        transactionManager.commit(transactionStatus);
      }
    }

    if (insertedRowStatus == null)
    {
	    insertedRowStatus = new CustomData<ApplicationMessage>();
    }

    insertedRowStatus.appendMessages(messagesHandler);

    return insertedRowStatus;
  }


  @Override
  public CustomData<ApplicationMessage> put(ApplicationMessage applicationMessage)
  {
    return this.put(applicationMessage, HandleTransaction.NO);
  }


  @Override
  public CustomData<ApplicationMessage> put(ApplicationMessage applicationMessage, HandleTransaction handleTransaction)
  {
    TransactionStatus transactionStatus = null;

    MessagesHandler messagesHandler = new MessagesHandler();

    if (handleTransaction == HandleTransaction.YES)
    {
      DefaultTransactionDefinition dtt = new DefaultTransactionDefinition();
      transactionStatus = this.transactionManager.getTransaction(dtt);
    }

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
       		"com.example.demo.general.framework.ApplicationMessageServiceImpl.put.messagesBeforeUpdate",
      		this.handleMessages(applicationMessage, MessagesPoint.BeforeUpdate));
    }

    CustomData<ApplicationMessage> updatedRowStatus = null;

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
       		"com.example.demo.general.framework.ApplicationMessageServiceImpl.put.messagesUpdate",
          (updatedRowStatus = this.applicationMessageDAO.update(applicationMessage.unpivot())));
    }

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
       		"com.example.demo.general.framework.ApplicationMessageServiceImpl.put.messagesAfterUpdate",
      		this.handleMessages(applicationMessage, MessagesPoint.AfterUpdate));
    }

    CustomData<ApplicationMessage> refetchedRow = null;

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
       		"com.example.demo.general.framework.ApplicationMessageServiceImpl.put.messagesRefetchRow",
      		(refetchedRow = this.get(applicationMessage.getApplicationMessageId())));
    }

    if (handleTransaction == HandleTransaction.YES)
    {
      if (messagesHandler.markedForRollback())
      {
        transactionManager.rollback(transactionStatus);
      }
      else
      {
        transactionManager.commit(transactionStatus);
      }
    }

    CustomData<ApplicationMessage> returnRow = new CustomData<ApplicationMessage>();

    if (refetchedRow != null) returnRow.setData(refetchedRow.getData());
    returnRow.appendMessages(messagesHandler);

    return returnRow;
  }


  @Override
  public CustomData<ApplicationMessage> patch(Long id, JsonPatch jsonPatch)
  {
    return this.patch(id, jsonPatch, HandleTransaction.NO);
  }


  @Override
  public CustomData<ApplicationMessage> patch(Long id, JsonPatch jsonPatch, HandleTransaction handleTransaction)
  {
    TransactionStatus transactionStatus = null;

    MessagesHandler messagesHandler = new MessagesHandler();

    if (handleTransaction == HandleTransaction.YES)
    {
      DefaultTransactionDefinition dtt = new DefaultTransactionDefinition();
      transactionStatus = this.transactionManager.getTransaction(dtt);
    }

    if (handleTransaction == HandleTransaction.YES)
    {
      if (messagesHandler.markedForRollback())
      {
        transactionManager.rollback(transactionStatus);
      }
      else
      {
        transactionManager.commit(transactionStatus);
      }
    }

    UnpivotedData unpivotedData = new UnpivotedData(id, jsonPatch, this.applicationMessageDAO.getDataColumnDefinitions());

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
       		"com.example.demo.general.framework.ApplicationMessageServiceImpl.patch.messagesBeforeUpdate",
      		this.handleMessages(unpivotedData, MessagesPoint.BeforeUpdate));
    }

    CustomData<ApplicationMessage> updatedRowStatus = null;

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
       		"com.example.demo.general.framework.ApplicationMessageServiceImpl.patch.messagesUpdate",
          (updatedRowStatus = this.applicationMessageDAO.update(unpivotedData)));
    }

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
       		"com.example.demo.general.framework.ApplicationMessageServiceImpl.patch.messagesAfterUpdate",
      		this.handleMessages(unpivotedData, MessagesPoint.AfterUpdate));
    }

    CustomData<ApplicationMessage> refetchedRow = null;

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
       		"com.example.demo.general.framework.ApplicationMessageServiceImpl.patch.messagesRefetchRow",
      		(refetchedRow = this.get(id)));
    }

    CustomData<ApplicationMessage> returnRow = new CustomData<ApplicationMessage>();

    if (refetchedRow != null) returnRow.setData(refetchedRow.getData());
    returnRow.appendMessages(messagesHandler);

    return returnRow;
  }


  @Override
  public CustomData<CustomDummy> delete(Long id)
  {
    return this.delete(id, HandleTransaction.NO);
  }


  @Override
  public CustomData<CustomDummy> delete(Long id, HandleTransaction handleTransaction)
  {
    TransactionStatus transactionStatus = null;

    MessagesHandler messagesHandler = new MessagesHandler();

    if (handleTransaction == HandleTransaction.YES)
    {
      DefaultTransactionDefinition dtt = new DefaultTransactionDefinition();
      transactionStatus = this.transactionManager.getTransaction(dtt);
    }

    if (handleTransaction == HandleTransaction.YES)
    {
      if (messagesHandler.markedForRollback())
      {
        transactionManager.rollback(transactionStatus);
      }
      else
      {
        transactionManager.commit(transactionStatus);
      }
    }

    UnpivotedData unpivotedData = new UnpivotedData();
    unpivotedData.add("applicationMessageId", "Long", id);

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
          "com.example.demo.general.framework.ApplicationMessageServiceImpl.delete.messagesBeforeDelete",
          this.handleMessages(unpivotedData, MessagesPoint.BeforeDelete));
    }

    CustomData<CustomDummy> deletedRowStatus = null;

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
          "com.example.demo.general.framework.ApplicationMessageServiceImpl.delete.messagesDelete",
          (deletedRowStatus = this.applicationMessageDAO.delete(id)));
    }

    if (messagesHandler.proceed() && messagesHandler.markedForRollback() == false)
    {
      messagesHandler.addMessages(
          "com.example.demo.general.framework.ApplicationMessageServiceImpl.delete.messagesAfterDelete",
          this.handleMessages(unpivotedData, MessagesPoint.AfterDelete));
    }

    CustomData<CustomDummy> returnRow = new CustomData<CustomDummy>();

    returnRow.appendMessages(messagesHandler);

    return returnRow;
  }


}
